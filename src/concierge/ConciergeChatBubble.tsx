    ...
    >
      ...
      {!isSystem && (
        <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 mt-1 ${
          ...
          {isUser ? <User size={13} className="text-gray-300" /> : <Bot size={13} className="text-white" />}
        </div>
      )}
      ...
      <div className={`flex flex-col ${isUser ? 'items-end' : 'items-start'} max-w-[85%] min-w-0`}>
        ...
        <div className={`rounded-2xl px-3.5 py-2.5 text-[12px] sm:text-[13px] leading-relaxed whitespace-pre-line break-words ${
          isSystem
            ? 'bg-amber-950/30 border border-amber-700/30 text-amber-200/80 text-[11px] text-center px-4 rounded-full'
            : isUser
              ? 'bg-gray-700/50 border border-gray-600/40 text-gray-200 rounded-br-sm'
              : 'bg-[#0B251E] border border-[#123E32] text-gray-200 rounded-bl-sm'
        }`}>
          {isTyping ? (
            ...
            {actions.map((action, i) => (
              <button
                ...
                className="flex items-center gap-1.5 text-[10px] sm:text-[11px] font-bold
                  bg-emerald-950/40 hover:bg-emerald-900/50 border border-emerald-700/40
                  text-emerald-300 hover:text-emerald-200 rounded-xl px-3 py-1.5
        ...
        {/* Timestamp */}
        {timestamp && (
          <span className={`text-[9px] text-gray-600 mt-1 px-1 ${isUser ? 'text-right' : 'text-left'}`}>
            {timestamp}
          </span>
        )}
      </div>
